...
interface PostUpdateEditorProps {
  UpdateFormRef: RefObject<HTMLDivElement>;
}
const PostUpdateEditor: React.FC<PostUpdateEditorProps> = ({
  UpdateFormRef,
}) => {
  ...
  useEffect(() => {
    if (Object.keys(postData.editPost).length > 0) {
      const updateCategory = postCategory.data.find(
        (obj) => obj.id === postData.editPost.post_category
      );
      if (updateCategory) {
    ...
    <div ref={UpdateFormRef}>
      ...
      <Grid container justifyContent={"end"} my={2}>
        <CustomButton
          variant="contained"
          size="large"
          onClick={() => handleUpdatePost(postData.editPost.id)}
        >
